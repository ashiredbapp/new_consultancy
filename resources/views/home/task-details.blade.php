<div class="font-dark bold font-green">Today's Task(s)</div>
<div class="scroller" style="min-height:185px;">
    <!-- START TASK LIST -->
    <table class="table table-sm">
        ...
        <tbody>
        @forelse($my_tasks->where('start_date', '<=', $currentDate)->where('end_date', '>=', $currentDate) as $task)
        @php
            $_stausArray = ['ToDo', 'Started', 'Completed', 'SIT', 'UAT', 'Pre Production', 'Production']
        @endphp
                <tr data-task="{{$task->id}}">
                    ...
                    <td style="padding:1px;">{{$auth_cells->where('id',$task->cell_id)->first()->name??''}}</td>
                    <td style="padding:1px;">
                        <select style="padding:1px;" name="sprint" class="btn btn-mini sprint">
                            ...
                            <option style="padding:1px;" value="{{ $sprint->id}}"{{ $sprint->id==$task->sprint_id?'selected':'' }}>Sprint {{$sprint->name}}</option> 
                            @endforeach
                        </select>
                    </td>
                    <td style="padding:1px;">
                        <select data-id="{{$task->id}}" style="padding:1px;" name="dev_status" class="btn btn-mini status">
                    ...
                    <td class="actual" style="padding:1px;text-align:center;">
                        <select data-id="{{$task->id}}" style="padding:1px;" name="actual" class="btn btn-mini actuals">
...
<br/><br/>
<div class="font-dark bold font-green">Pending Task(s)</div>
<div class="scroller" style="min-height:185px;">
    <!-- START TASK LIST -->
    <table class="table table-sm">
        <thead>
        ...
        <tbody>
            @forelse($my_tasks->where('end_date', '<', $currentDate) as $task)
            @php
            $_stausArray = ['ToDo', 'Started', 'Completed', 'SIT', 'UAT', 'Pre Production', 'Production']
            @endphp
                <tr data-task="{{$task->id}}">
                    ...
                    <td>
                    <button id="{{ $task->id }}" class="btn btn-circle btn-primary btn-xs update" aria-expanded="false">
                        Update <i style="color:#39ff14;" class="fa fa-check"></i>
                    </button>
                    </td>
                </tr>
